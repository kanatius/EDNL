...
def solve_tsp_dynamic_programming(
    ...
    #a chave será uma tupla de (int, range()) que guarda um inteiro. ex: [(0), {1,2,3,4,5,6..,n-1}]
    memo: Dict[Tuple, int] = {}
    ...
    def dist(ni: int, N: frozenset) -> float:
        ...
        costs = [
            (nj, distance_matrix[ni, nj] + dist(nj, N.difference({nj}))) #a função dist usa memória cache se nj e N já foram passados, retorna valor da cache
            for nj in N #para cada linha, retorna (distancia de 1 ate 2 + menor distancia de 2 até {3,4,5,6,.., etc}
        ]
        
        #função para pegar o custo mínimo
        nmin, min_cost = min(costs, key = lambda x: x[1])

        #salva o nó com de menor caminho total disponível
